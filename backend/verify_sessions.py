...
def main() -> None:
    # 1. Create a child
    child = client.post(
        "/api/children", json={"name": "Session Test Child", "preferred_language": "ar"}
    ).json()
    cid = child["id"]
    print(f"Created child #{cid}: {child['name']}")

    # 2. Start a session
    start = client.post(f"/api/children/{cid}/sessions/start").json()
    ...
    reread = client.get(f"/api/sessions/{sid}/summary").json()
    ...
    again = client.post(f"/api/sessions/{sid}/end").json()
    ...
    recent = client.get(f"/api/children/{cid}/sessions").json()
    ...
    level_goal = client.post(
        ...
        json={"skill_id": level_goal_skill, "target_level": 2},
    ).json()
    practice_goal = client.post(
        f"/api/children/{cid}/goals", json={"skill_id": practice_goal_skill}
    ).json()
    print(
    ...
    for i in range(MAX_ATTEMPTS):
        ...
        if not (level_done and practice_done):
            goals = {g["id"]: g for g in client.get(f"/api/children/{cid}/goals").json()}
    ...
    goals_final = {g["id"]: g for g in client.get(f"/api/children/{cid}/goals").json()}
    ...
    archived = client.patch(
        f"/api/goals/{practice_goal['id']}", json={"status": "archived"}
    ).json()
